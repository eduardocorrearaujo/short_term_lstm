...
def plot_train(ax, indice, Ydata, df_predicted, df_predicted25, df_predicted975, split_point, label):
    
    ax.plot(indice[len(indice)-Ydata.shape[0]:split_point +len(indice)-Ydata.shape[0]], Ydata[:split_point, -1], 'k-', alpha=0.7, label='data')
    ax.plot(indice[len(indice)-Ydata.shape[0]:split_point + len(indice)-Ydata.shape[0] ], df_predicted.iloc[:split_point,-1], 'r-', alpha=0.5, label='median')
        

    ax.fill_between(indice[len(indice)-Ydata.shape[0]:split_point+len(indice)-Ydata.shape[0]], df_predicted25[df_predicted25.columns[-1]][:split_point],
                       df_predicted975[df_predicted975.columns[-1]][:split_point],
                       color='b', alpha=0.3)
...
def plot_test(ax, indice, Ydata, df_predicted, df_predicted25, df_predicted975, split_point, label):

    ax.plot(indice[split_point + len(indice)-Ydata.shape[0]:], Ydata[split_point:, -1], 'k-', alpha=0.7, label='data')
    ax.plot(indice[split_point+ len(indice)-Ydata.shape[0]:], df_predicted.iloc[split_point:,-1], 'r-', alpha=0.5, label='median')
        

    ax.fill_between(indice[split_point + len(indice)-Ydata.shape[0]:], df_predicted25[df_predicted25.columns[-1]][split_point:],
                        df_predicted975[df_predicted975.columns[-1]][split_point:],
                        color='b', alpha=0.3)
...
def plot_train_test(indice, Ydata, df_predicted, df_predicted25, df_predicted975, split_point, label):

    fig, ax = plt.subplots(1,2, figsize=(12,4.5))

    plot_train(ax[0], indice, Ydata, df_predicted, df_predicted25, df_predicted975, split_point, label)

    plot_test(ax[1], indice, Ydata, df_predicted, df_predicted25, df_predicted975, split_point, label)

    plt.savefig(f'{MAIN_FOLDER}/plots/train_val_{label}.png',bbox_inches='tight',  dpi = 300)
...
def plot_transf_predicted_vs_data(df_predicted_t, df_predicted, Ydata, indice, label, pred_window, factor, split_point=None, uncertainty=False, label_name = 'transf', label1 = 'transf', label2 = 'chik data'):
    ...
    P.legend()
    P.savefig(f'{MAIN_FOLDER}/plots/predictions/{label_name}.png',bbox_inches='tight',  dpi = 300)
    P.show()
