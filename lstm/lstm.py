...
from tensorflow.keras.layers import LSTM, Attention, Dense, Dropout, Bidirectional, Flatten, Conv1D, MultiHeadAttention, Concatenate #, ConvLSTM1D, TimeDistributed
...
def calculate_metrics(pred, y_true):
    """
...
def build_lstm_multi( hidden=8, features=100, predict_n=4, look_back=4, loss='msle', stateful = False, batch_size = 1,
    ...
    lstm_2 = LSTM(
        ...
        return_sequences=False,
    )(lstm_1, training=True)

    lstm_2 = Dropout(0.2)(lstm_2, training=True) 
...
def apply_model(city, ini_date='2021-01-01', end_train_date=None,
    ...
    Y_pred = inv_boxcox(Y_pred, 0.05) -1 
    ...
    if plot:
        plot_train_test(indice, Y_pred,  df_pred, df_pred25, df_pred975, len(X_train), city)

    metrics1 = calculate_metrics(np.percentile(pred, 50, axis=2), Y_pred)

    metrics2 = calculate_metrics(np.percentile(pred, 50, axis=2)[len(X_train):], Y_pred[len(X_train):])
